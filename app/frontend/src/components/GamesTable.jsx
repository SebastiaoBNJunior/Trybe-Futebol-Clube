...
// Componente funcional para exibir uma tabela de partidas
const GamesTable = ({ currentFilter, isAdm }) => {
  // Estado local para armazenar as partidas
  const [games, setGames] = useState([]);
  const navigate = useNavigate();

  // Função para obter as partidas com base no filtro atual
  const getGames = (endpoint) => requestData(endpoint)
    .then((response) => setGames(response))
    .catch((error) => console.log(error));

  // Efeito para carregar as partidas com base no filtro atual
  useEffect(() => {
  ...
  // Efeito para carregar as partidas quando a lista de partidas estiver vazia
  useEffect(() => {
    const endpoint = '/matches';
  ...
  // Se não houver partidas, exibe um componente de carregamento
  if (!games.length) {
    return (<Loading />);
  }
...
// Definição das propriedades esperadas pelo componente GamesTable
