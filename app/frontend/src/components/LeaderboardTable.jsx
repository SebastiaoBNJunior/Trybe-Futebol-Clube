...
// Componente funcional para exibir uma tabela de classificação
const LeaderboardTable = ({ currentFilter }) => {
  // Estado local para armazenar a classificação
  const [leaderboard, setLeaderboard] = useState([]);

  // Função para obter a classificação com base no filtro atual
  const getLeaderboard = (endpoint) => requestData(endpoint)
    .then((response) => setLeaderboard(response))
    .catch((error) => console.log(error));

  // Efeito para carregar a classificação com base no filtro atual
  useEffect(() => {
  ...
  // Efeito para carregar a classificação quando a lista estiver vazia
  useEffect(() => {
  ...
  // Se não houver dados de classificação, exibe um componente de carregamento
        ...
        <tbody>
          {
            ...
            index) => (
              <tr key={ uuidv4() }>
                {/* Detalhes da classificação de cada time */}
                <td
...
// Definição das propriedades esperadas pelo componente LeaderboardTable
